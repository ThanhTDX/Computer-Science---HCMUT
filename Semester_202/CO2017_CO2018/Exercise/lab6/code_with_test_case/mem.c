...
static void * best_fit_allocator(unsigned int size, int* block_size);
static void * first_fit_allocator(unsigned int size, int* block_size);
static void * worst_fit_allocator(unsigned int size, int* block_size);
...
int mem_init(unsigned int size) {
	...
	// create blocks as EX 3:
	free_regions = (struct mem_region *)malloc(sizeof(struct mem_region));
	free_regions->size = 300 ;
	free_regions->pointer = (char*)mem_pool;
	free_regions->prev = NULL;

	struct mem_region * tmp = free_regions;
	int arr[6] = {300 , 600 , 350 , 200 , 750 , 125 }, i;
	for (i = 1; i < 6; i++){
		struct mem_region* tmp_reg = (struct mem_region *)malloc(sizeof(struct mem_region));
		tmp_reg->size = arr[i];
		tmp_reg->pointer = (char *) (tmp->pointer + arr[i - 1]);
		tmp_reg->prev = tmp;
		tmp->next = tmp_reg;
		tmp = tmp_reg;
	}
...
void * mem_alloc(unsigned int size) {
	...
	int block_size;
	void * pointer = first_fit_allocator(size, &block_size);
	// void * pointer = best_fit_allocator(size, &block_size);
	// void * pointer = worst_fit_allocator(size, &block_size);
	
	// FOR VERIFICATION ONLY. DO NOT REMOVE THESE LINES
	if (pointer != NULL) {
		printf("Alloc [%4d bytes] in block [%d]\n", size, block_size);
	}else{
...
void * best_fit_allocator(unsigned int size, int* block_size) {
	int found = 0;
	...
	if (found) {
		...
		*block_size = current_region->size;
...
void * worst_fit_allocator(unsigned int size, int* block_size) {
	int found = 0;
	struct mem_region* current_region = free_regions;
...
void * first_fit_allocator(unsigned int size, int* block_size) {
	/* First fit example */	
	int found = 0;
	struct mem_region * current_region = free_regions;
