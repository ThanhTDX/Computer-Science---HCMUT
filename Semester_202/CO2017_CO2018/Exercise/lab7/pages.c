...
int translate(address_t virtual, address_t * physical) {
	...
	// to physical address.
	index_t v_index = get_index(virtual);
	offset_t v_offset = get_offset(virtual);
	//TLB hit - if test is found in TLB
	...
	{
		//Redirection
		if (page_table[i].virtual == tmp->virtual)
		{
			page_table[i] = *tmp;
			ctr++;
		}
		//successful find in page_table
