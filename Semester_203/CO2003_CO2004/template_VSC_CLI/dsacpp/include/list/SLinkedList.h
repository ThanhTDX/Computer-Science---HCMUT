...
T SLinkedList<T>::removeAt(int index){
    ...
    index = 2:
             ptr
              |
              v
    head->20->10->90->tail => head->20->10->tail
    */
    Node *ptr = head;
    int ctr = 0;
    while (ctr != index)
    ...
    delete ptr->next;
    /*NOTE: after deletion list will be like this
         ptr
          |             result -> 90
          v 
    head->20->10 tail (no next pointer to tail)
    => nextNode is necessary for the bridge
    */
    ptr->next = nextNode;
    count--;
