...
void Heap<T>::reheapUp(int position){
    ...
    if(comparator)
    {
        if(comparator(elements[position], elements[parent]) == -1)
        {
...
void Heap<T>::reheapDown(int position){
    //YOUR CODE HERE
    if (2*position >= count-1) return;
    int lChild = 2*position+1;
    int rChild = 2*position+2;
    int child;
    if(comparator)
    {   
        if (2*position+1 == count-1) child = lChild;
        else child = (comparator(elements[lChild], elements[rChild]) == -1) ? lChild : rChild;
        if(comparator(elements[position], elements[child]) == 1)
        {
            swap(position, child);
            reheapDown(child);
        }
    ...
    {
        if (2*position+1 == count-1) child = lChild;
        else child = (elements[lChild] < elements[rChild]) ? lChild : rChild;
