...
for path in ['./test/','./main/bkit/parser/','./main/bkit/utils/','./main/bkit/astgen/','./main/bkit/checker/','./main/bkit/codegen/']:
	sys.path.append(path)
# ANTLR_JAR = os.environ.get('ANTLR_JAR')
ANTLR_JAR = '../../../antlr-4.9.3-complete.jar'
TARGET_DIR = '../target'
GENERATE_DIR = 'main/bkit/parser'
...
def main(argv):
    if len(argv) < 1:
        ...
        if not os.path.isdir(TARGET_DIR + "/" + GENERATE_DIR):
            subprocess.run(["java","-jar",ANTLR_JAR,"-o",GENERATE_DIR,"-no-listener","-visitor","main/bkit/parser/BKIT.g4"])
