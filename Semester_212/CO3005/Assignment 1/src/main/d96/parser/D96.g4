...
attri_body	: id_list CL data_type | attri_recurse;
attri_recurse
			: (ID | STATIC_ID) CM attri_recurse CM expr
...
id_list		: (ID | STATIC_ID) (CM (ID | STATIC_ID))*;

//2.3 Method Declaration
method_decl	: (ID | STATIC_ID) LB param_list? RB block_stmt
...
//5 Expression
expr	: LITERAL | idx_arr | mul_dim_arr | SELF | NULL | ID | func_call
		| LB expr RB
		...
		;
idx_expr: ID STATIC_ACC STATIC_ID | expr DOT ID;
...
varval_const_stmt
		: (VAR | VAL) const_body SEMI;
const_body
		: ID (CM ID)* CL data_type | const_recurse;
const_recurse
		: ID CM const_recurse CM expr
		| ID CL data_type ASSIGN expr
		; 
//6.2 Assignment Statement
assign_stmt : (ID | idx_expr) ASSIGN expr SEMI;
...
idx_arr	: ARRAY LB 	( (FLOAT_LIT (CM FLOAT_LIT)*)  
					| (INT_LIT (CM INT_LIT)*) 
					| (BOOL_LIT (CM BOOL_LIT)*)  
					| (STR_LIT (CM STR_LIT)*)  
					)?
				RB ; 
